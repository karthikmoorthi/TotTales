...
export function StoryReader({
  ...
  const tapGesture = Gesture.Tap()
    .onEnd((event) => {
      ...
      const leftZone = (SCREEN_WIDTH - bookWidth) / 2 + bookWidth * 0.3;
      const rightZone = (SCREEN_WIDTH - bookWidth) / 2 + bookWidth * 0.7;

      if (event.x < leftZone) {
        runOnJS(goToPrevPage)();
      } else if (event.x > rightZone) {
        runOnJS(goToNextPage)();
      }
    });
    ...
    if (page.type === 'cover') {
      return (
        <View style={styles.coverPage}>
          ...
          <Text style={styles.swipeHint}>Tap or swipe to read</Text>
        </View>
      );
    }
